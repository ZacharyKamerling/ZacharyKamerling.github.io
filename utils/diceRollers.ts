...
export class DiceRoller {
    ...
    redisplay() {
        if (this.lastRolls !== null && this.lastStat !== null && this.lastLabel !== null) {
            ...
            const stress = this.character.stress || 0;
            const resolve = this.character.resolve || 0;
            const [d1, d2] = this.lastStressDice;
            const total = Math.max(0, Math.min(20, d1 + d2 + stress - resolve));
            let resultText = '';
            if (total <= 5) resultText = 'Steady Nerves - No effect as you steel yourself and press on.';
            else if (total <= 7) resultText = 'Shaken - Ignore the next Positive card result you take.';
            else if (total <= 9) resultText = 'Jittery - Lose 1 Stamina Token.';
            else if (total <= 11) resultText = 'Hesitation - You can\u2019t use abilities in your next round of combat.';
            else if (total <= 13) resultText = 'Lash Out - You bleed or wound an ally.';
            else if (total <= 15) resultText = 'Despair - You cannot reduce your Stress for 24 hours.';
            else if (total <= 17) resultText = 'Faint - Collapse. You can do nothing until one round passes.';
            else if (total <= 19) resultText = 'Heart Attack - Your heart stops and you collapse. You die in 2 rounds unless revived.';
            else resultText = 'Retirement - You\u2019ve seen enough. Your adventuring days are done.';
            this.resultBox.innerHTML = `
                <div class="dice-result-label">Stress Panic Roll</div>
                <div class="dice-result-row">
                    <span class="dice-face" data-idx="0" style="margin-right:0.5em; color:#fff; background:none; font-weight:bold; text-shadow:none; cursor:pointer;" title="Click to reroll">${d1}</span>
